...
生成系のタスクにおいて, 一般にnon-autoregressive(NAR) modelはautoregressive(AR) modelより動作が速いが出力文の品質が低いが, 特にTarget-token間の依存性が高いタスクほどNAR modelの性能は落ちやすいことを示し, NAR modelにとってのタスクの難易度を推定するCoMMA modelを提唱した
