...
会話形式のQAタスク(conversational QA, ConvQA)をspan extractionではなく生成で解く研究. ConvQAでは解答を完全な文で出力する必要があるが, QAデータセットの正答は文の形では与えられていない場合もある. そこで, 既存のQAデータセットの正答を完全な文となるように書き換えたデータセットを自動的に作成し, それをseq2seqタスクとして解くことにより高い性能を実現した
